...
class User:
    ...
    def user_mainmenu(self,a,b):
        ...
        mi = mi.subsample(a, b)
        self.aitems = Button(self.mainframe, text="Items",bd=5,font="Arial 11 bold",bg="cyan", image=mi, compound=TOP,command=self.builditemtable)
        self.aitems.image = mi
        self.aitems.place(x=260, y=17)
        mi = PhotoImage(file="images/invoice2.png")
        mi = mi.subsample(a,b)
        self.aitems = Button(self.mainframe, text="Invoice",bd=5,bg="cyan",font="Arial 11 bold", image=mi, compound=TOP,command=self.make_invoice)
        self.aitems.image = mi
        self.aitems.place(x=62, y=17)
        mi = PhotoImage(file="images/change1.png")
        mi = mi.subsample(a, b)
        self.changeuser = Button(self.mainframe, text="Sign out",bd=5,bg="cyan",font="Arial 11 bold", image=mi, compound=TOP)
        self.changeuser.image = mi
        self.changeuser.place(x=460, y=17)
        mi = PhotoImage(file="images/Door_Out-512.png")
        mi = mi.subsample(a, b)
        self.logout = Button(self.mainframe, text="Quit",bd=5,bg="cyan",font="Arial 11 bold", image=mi, compound=TOP)
    ...
    def user_input(self):
       ...
       Label(self.entryframe, text="Quantity", font="Arial 12 bold", bg="#ffffff").place(x=318,y=0)
       Label(self.entryframe, text="Search", font="Arial 12 bold", bg="#ffffff").place(x=100, y=0)
       Label(self.entryframe, text="Amount Due", font="Arial 14 bold", bg="#ffffff").place(x=0, y=205)
       self.cur.execute("select max(invoice) from sales")
       self.invoice = self.cur.fetchall()
       self.invoice = self.invoice[0][0] + 1
       Label(self.tableframe1, text="Invoice No. "+str(self.invoice), font="Arial 14 bold", bg="#ffffff").grid(row=0, column=0)
       ...
       va=0
       for i in range(0,4):
           Label(self.entryframe1, text=li[i], font="Arial 14 bold", bg="#FFFFFF").place(x=0, y=va)
           va += 65
       self.cartitemid = StringVar()
       self.cartitem = StringVar()
       self.cartitemprice = StringVar()
       self.cartitemstock = StringVar()
       Entry(self.entryframe1, textvariable=self.cartitemid, font="Arial 14", bg="#FFFFFF", width=25, state='readonly').place(x=162, y=0,height = 40)
       Entry(self.entryframe1, textvariable=self.cartitem, font="Arial 14", bg="#FFFFFF", width=25, state='readonly').place(x=162, y=65,height=40)
       Entry(self.entryframe1, textvariable=self.cartitemprice, font="Arial 14", bg="#FFFFFF", width=25, state='readonly').place(x=162,y=65*2,height=40)
       Entry(self.entryframe1, textvariable=self.cartitemstock, font="Arial 14", bg="#FFFFFF", width=25, state='readonly').place(x=162,y=65*3,height=40)
       self.id_qty=dict()
